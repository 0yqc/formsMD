...
def multiple_choice_answer(line: str, qid: str):
	...
	if aid == 'other':
		...
		label = line.strip()
		label = md.markdown(label).removeprefix('<p>').removesuffix('</p>')
		return f'<div id="{qid}_{aid}" class="answer text"><input type="radio" name="{qid}_{aid}" id="{qid}_{aid}_radio" style="visibility:hidden;"><label id="{qid}_{aid}_label">{label}</label></div>'


def dropdown_option(line: str, qid: str, count: int):
	if ':' in line:  # an id was set
		aid = line.split(')')[1].split(':')[0].strip()  # between ')' and ':', striped
		line = line.split(')')[0] + ')' + ':'.join(line.split(':')[1:])  # remove id from line
	else:
...
def dropdown(block: str):
	...
	while i < len(lines):  # alternate syntax to "for line in lines" to dynamically change the current line number
		if lines[i].startswith('>'):  # description lines
			...
			descr_started = False
		if lines[i].startswith(('|', '')):  # checkbox or paragraph
			answer_html += dropdown_option(lines[i].strip(), qid, aid)  # add checkbox line to the options
			aid += 1
		i += 1  # go to next line
	return f'<div id="{qid}" class="question dropdown"><h3 id="{qid}_title" class="question dropdown title">{title}</h3><div id="{qid}_description" class="question dropdown description">{descr}</div><select>{answer_html}</select></div>'
...
def text(block: str):
	...
	try:
		...
		value = ''
	if datalist:
		return f'<div id="{qid}" class="question text {inp_type}"><h3 id="{qid}_title" class="question text {inp_type} title">{title}</h3><div id="{qid}_description" class="question text {inp_type} description">{descr}</div><input type="{inp_type}" id="{qid}_input" name="{qid}" class="answer text {inp_type}" placeholder="{placeholder}" value="{value}" list="{qid}_datalist">{datalist_html}</div>'
	else:
		return f'<div id="{qid}" class="question text {inp_type}"><h3 id="{qid}_title" class="question text {inp_type} title">{title}</h3><div id="{qid}_description" class="question text {inp_type} description">{descr}</div><input type="{inp_type}" id="{qid}_input" name="{qid}" class="answer text {inp_type}" placeholder="{placeholder}" value="{value}"></div>'
...
def area(block: str):
	...
	try:
		...
		rows = '4'
	return f'<div id="{qid}" class="question text area textarea"><h3 id="{qid}_title" class="question text area title">{title}</h3><div id="{qid}_description" class="question text area description">{descr}</div><textarea id="{qid}_input" name="{qid}" rows="{rows}" placeholder="{placeholder}">{value}</textarea></div>'
