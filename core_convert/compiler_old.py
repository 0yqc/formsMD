...
def matrix(block: str):
	...
	while i < len(lines):  # alternate syntax to "for line in lines" to dynamically change the current line number
		...
		if lines[i].startswith('-'):
			None
		else:  # must be table-head line
			head_html = f'<tr>'
			cols = lines[i].split('|')
			for i in range(len(cols)):
				cols[i] = cols[i].strip()
				head_html += f'<td>{cols[i]}</td>'
			head_html += f'</tr>'
	return q_html
...
def text(block: str):
	...
	try:
		...
		accept = ''
	if datalist:
		return f'<div id="{qid}" class="question text {inp_type}"><label for="{qid}_input" id="{qid}_label"><h3 id="{qid}_title" class="question text {inp_type} title">{title}</h3></label><div id="{qid}_description" class="question text {inp_type} description">{descr}</div><input type="{inp_type}" id="{qid}_input" name="{qid}" class="answer text {inp_type}" placeholder="{placeholder}" value="{value}" list="{qid}_datalist" accept="{accept}">{datalist_html}</div>'
	else:
		return f'<div id="{qid}" class="question text {inp_type}"><label for="{qid}_input" id="{qid}_label"><h3 id="{qid}_title" class="question text {inp_type} title">{title}</h3></label><div id="{qid}_description" class="question text {inp_type} description">{descr}</div><input type="{inp_type}" id="{qid}_input" name="{qid}" class="answer text {inp_type}" placeholder="{placeholder}" value="{value}" accept="{accept}"></div>'
...
def g_options(block: str):
	...
	for i in range(len(option_lines)):
		option_lines[i] = option_lines[i].removeprefix('{').removesuffix('}')
		options.update(options_compile(option_lines[i]))
	return options
